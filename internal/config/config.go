...
type GitLab struct {
	...
	// This should only be set for GitLab self-hosted instances.
	// For example, "https://gitlab.mycompany.com/" (without "/api/v4" suffix).
...
func loadFromEnv() error {
	...
	} else if gitlabBaseURL := os.Getenv("GITLAB_BASE_URL"); gitlabBaseURL != "" {
		Av.GitLab.BaseURL = gitlabBaseURL
	}
